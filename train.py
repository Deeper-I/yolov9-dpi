...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%11s' * 8) % ('Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'dfl_loss', 'Instances', 'Size', 'LR'))
        if RANK in {-1, 0}:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                pbar.set_description(('%11s' * 2 + '%11.4g' * 6) %
                                     (f'{epoch}/{epochs - 1}', mem, *mloss, targets.shape[0], imgs.shape[-1], optimizer.param_groups[0]["lr"]))
...
def parse_opt(known=False):
    ...
    # parser.add_argument('--imgsz', '--img', '--img-size', nargs="+", type=int, default=640, help='train, val image size (pixels)')
    ...
    parser.add_argument('--optimizer', type=str, choices=['SGD', 'Adam', 'AdamW', 'LION', 'LAMB'], default='SGD', help='optimizer')
