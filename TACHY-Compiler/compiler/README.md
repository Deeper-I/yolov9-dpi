...
**Note**: The `*.out` files (such as `block_4bit.out`, `fc.out`) in the `utils/` directory are distributed separately and are not included in this repository. If you need to use TACHY-Compiler features that require these files, you must obtain them separately from the TACHY-Compiler distribution package.
