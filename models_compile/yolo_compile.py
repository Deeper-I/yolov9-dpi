    ...
    def __init__(
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                ...
                Conv(
                    x, c2, 3,
                    transform=transform, pruning=pruning,
                ),
                Conv(
                    c2, c2, 3, g=4,
                    transform=transform, pruning=pruning,
                ...
                cnn.Conv2d(
                    c2, 4 * self.reg_max, 1,
                    groups=4, dilation=1, bias=True,
                    use_transform=use_transform, bit=bit, max_scale=max_scale, 
                    use_pruning=False, prun_weight=prun_weight,
                ),
            ) for x in ch
        )
        self.cv3 = nn.ModuleList(
...
class DSegment(DDetect):
    ...
    def __init__(
        ...
        self.proto = Conv(
            ch[-1], self.nm, 1,
            transform=transform, pruning=None,
        )
...
class BaseModel(nn.Module):
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        #         m.forward = m.forward_fuse  # update forward
        # self.info()
        LOGGER.info('Skip Fusing layers... ')
        # for m in self.model.modules():
        #     if isinstance(m, (RepConvN)) and hasattr(m, 'fuse_convs'):
        #         m.fuse_convs()
        #         m.forward = m.forward_fuse  # update forward
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    LOGGER.info(f"\n{'':>3}{'from':>18}{'n':>3}{'params':>10}  {'module':<40}{'arguments':<40}{'optimize':<40}")
    ...
    for i, (f, n, m, args, opts) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            # Conv, ConvNA, DWConv, AConv, ConvTranspose,
            Conv, ConvNA, AConv, ConvTranspose,
            Bottleneck, BottleneckCSP,
            SPP, SPPF, SPPCSPC,
            nn.ConvTranspose2d, DWConvTranspose2d, ADown,
        ...
        elif m in {ConvRes, ConvResNA, DWConvRes}:
            c1 = args[0]
            c2 = args[1]
            if m in {DWConvRes}:
                args = [c1, c2, *args[2:]]
        ...
        # Optimize
        kwargs = {"transform": opts[0], "pruning": opts[1]} if opts is not None else None
        if kwargs is None:
        ...
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<40}{str(opts):<40}')  # print
