...
> **Note**: DEEPER-I is a modified version of the official [YOLOv9](https://github.com/WongKinYiu/yolov9) implementation with additional optimization features integrated via DDesignerAPI. The original YOLOv9 project is licensed under GNU GPL v3. See [LICENSE.md](LICENSE.md) for details.
...
DEEPER-I is a modified YOLOv9 implementation that extends the original YOLOv9 with advanced optimization training capabilities through DDesignerAPI integration.

## Additional Features

DEEPER-I includes the following enhancements over the original YOLOv9:

### DDesignerAPI Integration

This project integrates **DDesignerAPI** for advanced optimization training capabilities:
- **XWN (eXtended Weight Normalization)**: Enhanced weight normalization for improved training stability
- **DPI (Dynamic Pruning Integration)**: Dynamic pruning support for model optimization
- **Transform and Pruning Options**: Integrated transform and pruning options for model optimization

The DDesignerAPI is used in the following modules:
- `models/common.py`: Layer components with DDesignerAPI integration
- `models/yolo.py`: Model architecture with DDesignerAPI support
- `models/yolo_compile.py`: Compilation utilities with DDesignerAPI

**Note**: To use DDesignerAPI features, you need to have the DDesignerAPI package installed. Please refer to the DDesignerAPI documentation for installation and usage instructions.

### TACHY-Compiler Support

This project includes TACHY-Compiler integration for model compilation and deployment. However, **the `*.out` files in `TACHY-Compiler/compiler/utils/` directory are distributed separately** and are not included in this repository.

**Important**: If you need to use TACHY-Compiler features that require `*.out` files (such as `block_4bit.out`, `fc.out`), you must obtain them separately from the TACHY-Compiler distribution package.
...
DEEPER-I is licensed under the GNU General Public License v3.0 (GPL-3.0). See [LICENSE.md](LICENSE.md) for the full license text.

**Important Notes**:
- DEEPER-I is based on the official [YOLOv9](https://github.com/WongKinYiu/yolov9) implementation, which is also licensed under GPL-3.0
...
* Original YOLOv9: [https://github.com/WongKinYiu/yolov9](https://github.com/WongKinYiu/yolov9)
